...
<Steps>
  ...
  First, we need to create the contract that we will be using.
...
You will receive a status 201 with an object containing the ID of the contract.
  ...
  <Step title="Create an item type">
    Now, it's time to create a token type and link it to the contract that we've created. Think of a token type as the template for the token that you want to have. 
    
    <Info>Remember to switch `<CONTRACT_ID>` for the ID of the contract.</Info>
...
axios
  .post(
    "https://protocol-sandbox.lumx.io/contracts/<CONTRACT_ID>/token-types",
    {
      name: "First item type",
      description: "My first item type",
      initialSupply: 100,
...
You will receive a status 201 with an object containing the ID of the token type.
  ...
  <Step title="It's time to deploy it">
    ...
    ```javascript JavaScript
    axios
      .post("https://protocol-sandbox.lumx.io/contracts/<CONTRACT_ID>/deploy")
      .then((response) => {
        // Handle Success
        console.log(response);
      });
    ...
    **Congratulations!** You can check your contract on the [block explorer](https://amoy.polygonscan.com).
