...
<Steps>
  ...
  <Step title="Encode a transaction">
    ...
    Now, let's create an encoded transaction. We'll be using the custom transaction example from the [previous guide](/guides/recipes/invoke-a-custom-transaction), but the process for creating mints or transfers would be the same.
